...
source("01.Fonction_Simulation.R",encoding = "utf-8")
source("02.Fonctions_VEM.R", encoding = "utf-8")
source("03.Fonctions_graphique.R", encoding = "utf-8")
source("04.Fonction_evaluation.R", encoding = "utf-8")
source("05.Modeles_ref.R", encoding = "utf-8")
...
U <- list()
for(t in 1:nsignaux){
  ut <- matrix(nrow = d, ncol=(d*(p)))
  for(j in 1:(p)){
    ut[,j] <- diag(METEO[t,(j)], d)
  }  
  U[[t]] <- ut
}
...
BDD <- simulation_data_VEM_delta_2_melange(V_k, W_k,
                                   Sigma_0, Phi_k,
                                   mu_0, pi_simu,
                                   facteurs,
                                   coefficients_A, nsignaux, K=2, 
...
res <- algorithme_VEM(BDD, K=2, TT=nsignaux) 
